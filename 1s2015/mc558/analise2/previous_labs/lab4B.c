/*
    ...
    Laboratório 4 - Parte B - Pedágio Máximo
...
// functions 
void creat_list_edge(SVertex l[NMAX][NMAX],int from,int to,int weight,int pos);
...
int main(){
    ...
    static int source_dist[NMAX],dest_dist[NMAX];
    static int l_pos[NMAX],h_pos[NMAX]; // save the position for insertin in O(1)
	...
	for(i = 0; i < edges; i++){
        ...
        l_pos[from] += 1;
        h_pos[to] += 1;
        creat_list_edge(list,from,to,weight,l_pos[from]);
        creat_list_edge(h,to,from,weight,h_pos[to]);
    }
